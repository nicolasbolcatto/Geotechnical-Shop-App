...
//Create boolean variable for administrator rights
dotenv.config()
let admin = process.env.ADMIN || false
...
async function controllerProductsPost(req, res) {
    ...
    consoleLogger.info(`Route = /api/products${url} Method ${method}`)
    if (admin == false) {
        res.json({ error: -1, description: "Route /api/products Method POST not authorized" })
        return
    }
    try {
